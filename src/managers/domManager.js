...
DomManager = {
  Animate: {
    ...
    FadeInRight: function(variableToCheck, classOfElementsToAnimate, fastSlowOrDefault = "") {
      var ref, ref1;
    ...
    FadeInUp: function(variableToCheck, classOfElementsToAnimate, fastSlowOrDefault = "") {
      var ref, ref1;
      if (typeof variableToCheck === 'boolean') {
        if (variableToCheck === true) {
  ...
  HandleShareWithSelection: function(e, currentUser, shareWith) {
    var clickedEl, key;
    clickedEl = e.currentTarget;
    key = clickedEl.getAttribute('data-key');
    DomManager.toggleActive(clickedEl);
    if (indexOf.call(shareWith, key) >= 0) {
      shareWith = shareWith.filter(function(x) {
        return x !== key;
      });
    } else {
